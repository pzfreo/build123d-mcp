...
- **Composable analysis primitives in the `execute()` namespace (#366).** `measure`, `clearance`, `cross_sections`, and `find_holes` are now callable *in code* and return real Python objects (dicts / recogniser records), so an agent computes over results — `measure(part)["volume"]`, `[h for h in find_holes(bracket) if h.location.X < 5]` — instead of reading numbers out of one JSON tool result and re-typing them into the next call (every hand-copied number is a transcription-error opportunity; SpatialInterface/[SpatialClaw](https://arxiv.org/abs/2606.13673) traced ~half of code-vs-tool-call wins to exactly this composition). The MCP tools are unchanged. `measure`/`clearance`/`cross_sections` route through the same bounded-subprocess path as the tools, so a large shape can't SIGKILL the session (#360); they take a shape *variable* (by registered name awaits #286). First of a staged rollout — the remaining recognizers/`align_check` follow the same pattern.
