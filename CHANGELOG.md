...
- **Out-of-process tools no longer surface a bare `null` on the first call in a fresh session (cold-start).** A field report from the CADGenBench pipeline found the **first** invocation of a subprocess-backed tool (`locate_gate_defects`, `design_audit`, and by shared machinery `export`/`render_view`/`shape_compare`) returned `null` ~100% of the time in a cold container, with later calls succeeding — the model burned a tool round-trip learning to double-call, or silently skipped the tool. Two fixes: (1) **never surface `null`** — the worker-op proxy now catches a worker kill / op timeout / in-worker error and returns a structured `{"error": …}` result (as `execute()` already did), so the client always gets a readable, distinguishable reason instead of a bare `null`; this also makes the true cause visible in transcripts. (2) **Warm the out-of-process path at worker startup** — a daemon thread primes `build123d` in a fresh child so the first real subprocess-tool call isn't cold (the cold `from build123d import *` on a huge OCP install is what blew the first-call budget); best-effort, off the critical path, and skipped gracefully on hosts that block child processes. The low-level `_call` state-loss-on-timeout contract is unchanged. (field report #2)
