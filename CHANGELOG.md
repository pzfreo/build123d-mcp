...
- **`recover()` tool — heal an invalid solid, or fail without touching the geometry.** When `validate()` FAILs and hand-coded `ShapeFix` in `execute()` can't clear the defect (e.g. an unorientable or un-meshable face, often inherited from a malformed imported STEP), `recover()` runs a small heal ladder — `ShapeFix` → **defeature** the BRepCheck-invalid faces (remove them, neighbours close the gap) — and keeps the first variant whose **written-and-reimported STEP** (the artifact a CAD scorer sees) passes the *exact* gate. Two guarantees:
  - **Never distorts the part.** A heal is accepted only if it also preserves the geometry to within a **surface-deviation (Hausdorff) bound** — no surface point moves more than `min(max(1 mm, 1% of the diagonal), 3 mm)`. This is the invariant a bounding-box or signed-volume proxy could not enforce: defeaturing away an internal feature (e.g. a Ø16 through-bore) leaves the bounding box unchanged and the signed volume meaningless, but moves the bore wall ~16 mm — caught. The 3 mm cap keeps ε from growing past the few-mm a real counterbore moves the surface on large parts. So `recover()` either returns a faithful valid solid (re-registered) or fails and leaves the original exactly as it was — it never hands back a mangled solid that merely happens to be watertight. (A feature whose removal moves every surface point less than ε is the guard's irreducible tolerance — documented in `_recover_subprocess`.)
  - **Cannot kill the session.** Defeaturing a large solid is an un-interruptible native call that can run for minutes; run in-process it would block the worker past its op budget and the session would be lost. So the heal runs **out-of-process, hard-bounded by the op time budget** (mirroring how export bounds its mesh gate); if it overruns it is killed and `recover()` returns FAIL, session intact. The parent's own reimport of the healed STEP is also un-interruptible, so it runs only when enough budget remains for the cost the subprocess measured — otherwise `recover()` FAILs untouched and saves the heal to a path the agent can import on a fresh op.

  Requires a single solid (a multi-solid shape is refused rather than guessing which body to heal). Validated end-to-end against two benchmark editing fixtures whose input STEPs are themselves BRepCheck-invalid: one is recovered via defeature at a 1.2 mm surface shift (well under its 1.9 mm tolerance); the other, whose defeature can't finish in budget, is killed and FAILs untouched with the session intact.
