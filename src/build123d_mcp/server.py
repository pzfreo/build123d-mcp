...
def recover(object_name: str = "", store_as: str = "") -> str:
    """Heal an invalid solid so it passes the validity gate, when validate() FAILs and ShapeFix in execute() cannot clear the defect (e.g. an unorientable or un-meshable face, often inherited from a malformed imported STEP). Runs a small heal ladder — ShapeFix → defeature the BRepCheck-invalid faces (remove them, neighbours close the gap) — and keeps the first variant whose written-and-reimported STEP (the artifact a CAD scorer sees) passes the exact gate. Two guarantees: (1) it never distorts the part — a heal is accepted only if every bounding-box face also moves less than max(1mm, 1% of the diagonal), so recover() either returns a faithful valid solid or fails leaving the original exactly as it was; (2) it cannot kill the session — the heal runs out-of-process, hard-bounded by the op time budget, so a defeature that would run for minutes is stopped and recover() FAILs untouched instead of blocking the worker. On PASS it re-registers the healed solid; on FAIL (no heal cleared the gate, or it ran out of budget) the original is untouched — inspect the defect with measure()/render and rebuild the region in execute(). Requires a single solid. object_name: named object from show() (default: current shape). store_as: name to register the healed solid under (default: overwrite object_name / update the current shape)."""
