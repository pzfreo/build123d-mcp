...
def locate_gate_defects(session, object_name: str = "") -> str:
    ...
    ``open_edge`` / ``nonmanifold_edge`` (B-rep edge midpoint), and
    ``mesh_nonmanifold_edge`` (the self-touch a CAD scorer rejects). Empty list
    means the part passes the structural checks. object_name: named object from
    show() (default: current shape).
    """
    t0 = time.monotonic()
